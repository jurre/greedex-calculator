...
export function RightSideImage({
  ...
  return (
    <div className="relative hidden xl:flex xl:w-1/2">
      <AnimatedGroup
        className={cn(
          "relative flex h-full w-full flex-col gap-6 overflow-hidden rounded-3xl border border-border/40 bg-card/30 p-8 shadow-2xl backdrop-blur-xl",
        )}
        variants={{
          container: {
            visible: {
      ...
      >
        ...
        <ul className="relative z-10 grid gap-2 text-sm">
          {highlights.map((value, index) => (
            <li
              key={`${value}-${index}`}
              className="flex items-center gap-3 rounded-2xl border border-border/50 bg-muted/30 px-3 py-1.5 font-medium text-sm backdrop-blur-sm"
            >
        ...
        <div className="relative z-10 space-y-2">
          ...
          <p className="text-muted-foreground text-xs">{heroCaption}</p>
        </div>
        ...
        {/* Stats */}
        <div className="relative z-10 mt-auto grid gap-2 text-[0.65rem] uppercase tracking-[0.4em] sm:grid-cols-2">
          <span className="rounded-2xl border border-primary/50 bg-primary/10 px-3 py-1.5 text-center text-primary backdrop-blur-sm">
            {heroStatOne}
          </span>
          <span className="rounded-2xl border border-border/50 bg-muted/30 px-3 py-1.5 text-center text-muted-foreground backdrop-blur-sm">
            {heroStatTwo}
          </span>
        </div>
      </AnimatedGroup>
