...
export function RightSideImage({
  ...
  return (
    <div className="relative hidden w-full lg:flex lg:w-1/2">
      <AnimatedGroup
        className={cn(
          "relative flex w-full flex-1 flex-col gap-6 overflow-hidden rounded-3xl border border-border/40 bg-card/30 p-6 shadow-2xl backdrop-blur-xl sm:p-8",
        )}
        variants={{
          container: {
            visible: {
      ...
      >
        ...
        <ul className="relative z-10 grid gap-2 text-sm">
          {highlights.map((value, index) => (
            <li
              key={`${value}-${index}`}
              className="flex items-center gap-3 rounded-2xl border border-border/50 bg-muted/30 px-3 py-2 font-medium text-xs backdrop-blur-sm sm:text-sm"
            >
        ...
        <div className="relative z-10 space-y-2">
          <div className="flex items-center gap-2 font-semibold text-primary text-xs uppercase tracking-[0.4em]">
        ...
        <motion.div
          ...
          className="relative z-10 h-40 w-full overflow-hidden rounded-3xl border border-border/50 bg-muted/30 shadow-lg sm:h-48"
        >
          <Image
